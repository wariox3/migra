...
class RhuContratoTipo{
    ...
    /**
     * RhuContrato constructor.
     */
    public function __construct()
    {
    }

    public function migrarContratoTipo(){
        try{
            require_once('../../Conexion.php');
            $conexion = new Conexion();
            $vanadio=$conexion->conexion1();
            $columnas=array(
                'codigo_contrato_tipo_pk',
                'codigo_externo',
                'nombre',

            );
            $totalDatos=$vanadio->query("SELECT COUNT(*) as 'numeroRegistro' FROM rhu_contrato_tipo");
            $totalDatos->execute();
            ...
            while ($aux!==(int)$count) {
                ...
                codigo_externo,
                nombre
                 FROM rhu_contrato_tipo limit {$aux},{$limite}");
                $value="";
                ...
                if($value!=""){
                    $cromo->query("insert into rhu_contrato_tipo(
                codigo_contrato_tipo_pk,
